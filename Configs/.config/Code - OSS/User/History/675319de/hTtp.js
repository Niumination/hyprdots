...
const BooruPage = (taglist, serviceName = 'Booru') => {
    ...
    const PreviewImage = (data, delay = 0) => {
            ...
            attribute: {
                'update': (self, data, force = false) => {
                    ...
                    const showImage = () => {
                        ...
                        self.queue_draw();
                        imageRevealer.reveal_child = true;
                    }
                    // Show
        ...
        const imageRevealer = Revealer({
            ...
            transitionDuration: userOptions.animations.durationLarge,
            child: EventBox({
                onHover: () => { imageActions.reveal_child = true },
                onHoverLost: () => { imageActions.reveal_child = false },
                child: imageOverlay,
            })
        })
    ...
    const thisPage = Box({
        ...
        attribute: {
            'imagePath': '',
            'isNsfw': true,
            'update': (data, force = true) => { // TODO: Use columns. Sort min to max h/w ratio then greedily put em in...
                ...
                Utils.timeout(IMAGE_REVEAL_DELAY,
                    () => pageImageRevealer.reveal_child = true
                );
...
export const booruCommands = Box({
    ...
    setup: (self) => {
        ...
        self.pack_start(Button({
            ...
            label: 'Tags →',
            onClicked: () => {
                booruTags.reveal_child = !booruTags.reveal_child;
            }
        }), false, false, 0);
