...
import waifu from "./AIService.js";
...
try {
  ...
  /**
  * @param {import('modules/waifu/AIService').waifuMessage} msg
  ...
  */
  ...
  const TextEntry = (fileContainer) => {
    ...
    const textView = Widget.TextView({
      ...
      .hook(QSState, (entry) => {
        if (QSState.value === "waifu")
          entry.grab_focus();
      })
      .hook(App, (entry, window, visible) => {
        if (window === "quicksettings" && visible && QSState.value === "waifu")
          entry.grab_focus();
      });
  ...
  console.warn("webkit2gtk-4.1 is not installed. waifu module has been disabled.");
}

/**
 * @returns {import('types/@girs/gtk-3.0/gtk-3.0').Gtk.Widget}
 */
const QSwaifu = () => QuickSettingsPage(
  Menu({
    title: "waifu",
    icon: icons.ai,
    content: AIContainer(),
