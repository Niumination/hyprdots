...
function toggle(value) {
  const current = QSState.value;
  if(current == value && App.getWindow("quicksettings").visible) App.closeWindow("quicksettings");
  else {
    App.openWindow("quicksettings");
    QSState.value = value;
  }
}

globalThis.toggleQuickSettings = () => toggle("quicksettings");
...
//    exclusivity: "exclusive",
  ...
  const addKeyBinds = (mods) => {
    ...
    window.keybind("Shift Tab", () => QSState.next());
    for (let i = 0; i < 10; i++) {
